#!/usr/bin/env bash
# Launches one accuracy-session server: production's settings (run-nvfp4-e16c-public.sh as of 09-13 21:20:
# E16c policy, 65,536-token context, thinking on by default) from the cc-fp8 worktree, on 127.0.0.1:<port>,
# with the hot-cache budget and online FP8 selection given on the command line. The harness applies the
# chat template itself with thinking off, so the server default does not affect the measurements.
# Usage: run-fp8-server.sh <name> <port> <hot_gpu_mb> <groups|none> <scheme> [layers]
...
port=${2:?port}
hot_mb=${3:?hot_gpu_mb}
groups=${4:?groups or none}
scheme=${5:?scheme}
layers=${6:-}

work=/data/models/slang/nvfp4-work
worktree=$work/cc-fp8/worktree
...
run_dir=$work/cc-fp8/servers/$name/run-$(date +%Y%m%d-%H%M%S)
...
fp8_env=()
if [ "$groups" != none ]; then
    fp8_env=(SGLANG_ONLINE_FP8_GROUPS="$groups" SGLANG_ONLINE_FP8_SCHEME="$scheme")
    [ -n "$layers" ] && fp8_env+=(SGLANG_ONLINE_FP8_LAYERS="$layers")
fi

mkdir -p "$run_dir/profiles" "$work/runtime-tmp"
ln -sfn "$run_dir" "$work/cc-fp8/servers/$name/latest"
cd "$worktree"
{
    echo "cc-fp8 server $name port=$port hot_mb=$hot_mb groups=$groups scheme=$scheme layers=${layers:-all}: $(date --iso-8601=seconds)"
    git status --short --branch
    git log -1 --oneline
    sha256sum python/sglang/srt/layers/quantization/online_fp8.py python/sglang/srt/layers/quantization/modelopt_quant.py python/sglang/srt/layers/hyperconnection.py python/sglang/srt/models/qwen4_exp.py
    echo "FP8_ENV=${fp8_env[*]:-none}"
} 2>&1 | tee -a "$log"
