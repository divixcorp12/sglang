...
class Arm:
    def __init__(self, root, name, trace):
        root.mkdir()
        ...
        if trace:
            self.host.enable_trace(capacity=1 << 16)
        self.cursor = 0
    ...
    def block(self, rows_per_request, requests):
        ...
        windows = EXPERTS // rows_per_request
        t0 = time.perf_counter_ns()
        for _ in range(requests):
            w = self.cursor % windows
            self.cursor += 1
            ids = [w * rows_per_request + j for j in range(rows_per_request)]
...
def main():
    ...
    ap.add_argument("--rows", type=int, nargs="+", default=[1, 3, 8, 16])
    ...
    arms = {"off": Arm(base / "off", "off", False), "on": Arm(base / "on", "on", True)}
    conditions = {"before": _conditions()}
    result = {"conditions": conditions, "by_rows": {}}
    try:
        for rows in args.rows:
            for arm in arms.values():
                arm.block(rows, args.requests)  # warmup: page cache, ring, branch predictors
            samples = {"off": [], "on": []}
            served_rows = []
            loads = []
    ...
    for rows, r in result["by_rows"].items():
        print(
            f"rows={rows:>2} read/request={r['rows_read_per_request']:.1f} off={r['off_ns_median']:.0f} ns "
