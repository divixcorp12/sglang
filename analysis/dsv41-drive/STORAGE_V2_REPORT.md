...
with a non-atomic read-modify-write (`LEASE_PROTOCOL.md` OPEN 9, by reading, not run).
...
|---|---|---|---|---|
| **Demand and advisory sequence wrap.** The device skips sequence 0 on wrap (`post` kernel and `sim_post`: `if (seq == 0) seq = 1`); the service advances with `next_demand_ += 1u` and `next_advice_` likewise, with no skip (`exl3_ram_miss_host.cpp`, `pump_demand` / `pump_advice`, still present at HEAD) | **by reading at HEAD (confirmed)**; the team lead reports it **confirmed by execution** with `test/registered/unit/kernels/test_exl3_ram_miss_wrap.py`, which is **untracked and uncommitted**. I did not run it | only after 2^32 requests. At an assumed 160 requests/s (about 40 layers at 4 tok/s, an assumption not a measurement) that is about 310 days of continuous serving; the advisory ring may wrap sooner if it carries more requests (not measured) | the service spends one iteration on a phantom sequence 0: on a used page one spurious overrun (demand) or skipped advisory and a done word stepping back to 0; on a fresh page the phantom is read as an empty request and **served**. Any generation scheme layered on `next_demand_` would be off by one | `LEASE_PROTOCOL.md` D6, OPEN 1; not fixed |
| **`lanes <= 8` is not enforced at attach.** `Exl3RamMissRowBackend.translate` bounds lanes by `planned.numel() = max(capacity, MAX_IDS)` (`srt/layers/moe/exl3_ram_miss.py`), while the post kernel reads `min(count, kMaxIds = 8)` lanes (`exl3_ram_miss.cuh`) | by reading | only if a backend is built with `capacity > 8`; `graph_gather_rows == top_k` is required and **DSV4.1's top-k was not checked** (`LEASE_PROTOCOL.md` OPEN 2) | lanes past 8 are never requested from the service; the wait kernel then finds them missing and raises a fatal (`unserved_misses`), so it fails stop rather than silently | `LEASE_PROTOCOL.md` D7; not fixed |
...
These are the design document's findings about the code as it stands, established **by reading**; none was run except D6's test above. Numbering note: `PROMOTION_ASYNC.md` uses its own D1-D3, which are different defects (section 4.3). The severity column is
**as the source document characterises each defect**. The team lead refers to *corrected* severities; **I could not find them in any committed file, so I have not invented them: fill the last column when they are supplied.**

| id | defect | reachable today (source's words) | source's characterisation | corrected severity |
|---|---|---|---|---|
| D1 | on a wait timeout the kernel raises fatal but still runs the translate loop and the copy | after a timeout | "containment by two side effects (`keep = 0`, watchdog abort), not a design guarantee"; the plan itself says `keep = 0` alone does not protect an earlier gather | *(to supply)* |
| D2 | the device resolves slots through the mutable service-written slot map | "I could not construct a non-fault interleaving in today's code that violates" the exclusion | an entry that changes between the wait's read and the gather makes the copy read another expert's bytes while every counter reports success | *(to supply)* |
| D3 | no GPU-to-CPU acknowledgement exists | always | a missing capability, not a wrong result today | *(to supply)* |
| D4 | the wait kernel cannot be interrupted (fatal checked once on entry) | on shutdown or a late fatal | holds its stream until `timeout_ns` (default 2,000 ms); prompt shutdown impossible | *(to supply)* |
| D5 | shutdown does not establish that GPU readers finished, and a failed `cudaHostUnregister` frees memory anyway; a second exit-time finalizer unregisters slabs in every process | "the D5 hazard occurs in every normal exit today" **if** the finalizer order is unfavourable (OPEN 13, undetermined) | may free registered memory still being read | *(to supply)* |
| D6 | the demand sequence wrap (section 4.1) | after 2^32 requests | one spurious overrun per wrap; corrupts a layered generation scheme | *(to supply)* |
| D7 | lane capacity silently 8 (section 4.1) | only if a backend has `capacity > 8` | lanes past 8 unserved, then fatal | *(to supply)* |
...
- **Needed from the owner:** the corrected severities for D1-D7 (section 4.2); the decisions listed in `LEASE_PROTOCOL.md` §19 and `PROMOTION_ASYNC.md` §14; whether to add Task 3's deferrals to the plan's Task 9 table.
