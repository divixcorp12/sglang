...
| Task 6: saving 19.2 ms/step, 87 % hit lanes, per-row over two-phase +5.09 ms best / -14.35 ms random order | **model**, one workload (n_eff = 1; per-session range 5.3-8.7 %); arithmetic independently reproduced | A2, c = 1.055 ms, launch 1.0 ms assumed; A1 now bounded (10.8-63.6 hit lanes per step in read layers, two-phase about 4 % at the worst placement) | `per_row_precheck_result.txt`, `PER_ROW_PRECHECK_REVIEW.md` |
