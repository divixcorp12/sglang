...
| file | sha256 |
|---|---|
| `c_measurement/c_analysis.py` (gates, `T(n)` fits, the model recompute on measured lanes, the label; `--selftest` produces STANDS / INTERMEDIATE / WITHDRAWN and five INVALID cases from synthetic `T(n)` with known `c`, needs the divix01 traces) | `6e25caf7aa3d2304ab8a359ad40dc0773cffcfaeee1d551d98754e53284c37f9` (**IN FORCE: amendment 11, section 22.2**; supersedes `21afc941...` (amendment 10), `b93ebf7c...` (amendment 9) and `4657702c...`, the constant cited elsewhere in this document) |
...
| `c_measurement/test_c_harness.py` (the harness-to-analysis hand-off, plus the two guard tests of section 17a) | `c6e23835810dc70dc4a22abc06a1eedeafceee4567949489ddb763e1a29baa27` (**IN FORCE: amendment 11, section 22.2**; 27 CPU tests; supersedes `5dabd036...` and `b6815a80...`) |
...
### 22.2 Amendment 11: the marker refusal is scope-aware (the lead's ruling, 2026-09-21, before any run under amendment 10)

**What the ruling corrects.** Section 22.1 made `c_analysis.py` refuse beside *any* `results.INVALID` marker, on the lead's instruction. The implementer pointed out that this collides with section 21 item 2, which left a drive-traffic marker alone because section 12 item 3(b) scopes it to the `nvme` arm's `rho`; with `--with-nvme`, a drive-drift marker would void a `T(n)` verdict the registration says it does not touch. The lead ruled that the refusal must follow the registered scope. This is a change to a frozen file, disclosed as one, and it moves no gate, threshold, arm, statistic or verdict rule: **it can only withhold numbers or print a scope note, never admit a value the gates reject.**

**The rule as implemented.** `c_analysis.py` reads `results.INVALID` before its input. If the text is *scoped* (every non-empty line contains one of `nvme arm`, `nvme ratio`, `load arm`, `rho`; these are the phrases carried by all four texts `c_harness.py` writes: drive traffic in a load window, a load arm below its reader floor, box drift between the idle and load arms, a dirty idle baseline) it proceeds: line 1 is still the verdict word, **line 2 is the scope note** (the marker text and "the nvme arm's `rho` is NOT to be quoted"), and no fit, model or flag line about a `load == "nvme"` arm is printed. The gates, including those on the nvme cells, are unchanged, so a failing gate is still INVALID. If the text is anything else the run is voided: `VERDICT: REFUSED`, the marker text, exit 3, and nothing analysed. **The classifier fails closed**: an unrecognised text voids the run. A marker is never renamed or removed. `c_intercept_report.py` follows the same rule.

**Also disclosed.** The lead has seen the implementer's preliminary ungated numbers (`gather_tn.py`, `task6-microbench/data/`), as has the implementer; the ruling is about scoping and depends on no value.

**Tests.** `--selftest` still passes all 15 cases; `test_c_harness.py` is 27 tests (the marker test is split in two: an unrecognised marker refuses and prints no fit; each of the four harness texts is recognised as scoped and a scoped marker beside real dry-run output proceeds, prints the scope on line 2, prints the sm/cold/idle fit and prints nothing containing `nvme` after it). Run on divix01, CPU: selftest PASSED, 27 passed.

**Hashes (in force after the section-9 flip in the next commit).** `c_analysis.py`: old `21afc941c29f95be50613187ccd91072a58b2e8372c69661e7f07c74cea26e92` (amendment 10), new `6e25caf7aa3d2304ab8a359ad40dc0773cffcfaeee1d551d98754e53284c37f9`. `test_c_harness.py`: old `5dabd036a204e71ba9dacea51914dd212df70ea5c115138be963d64a43c7fcc1`, new `c6e23835810dc70dc4a22abc06a1eedeafceee4567949489ddb763e1a29baa27`. `c_harness.py` is unchanged (`b2e71474...`).
