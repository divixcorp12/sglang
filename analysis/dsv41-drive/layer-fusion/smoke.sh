#!/usr/bin/env bash
# Layer-fusion smoke on divix01: one cold server per arm on the arm_env recipe (100 GiB tier, two-phase, piece stream,
# row images), stage-traced, six greedy decode requests (prompts 0-2, twice). Writes env.txt, driver.log, server.log,
# stages.jsonl and responses.jsonl to $OUT/<tag>-<arm>. Derived from analysis/dsv41-drive/copy-overlap/smoke.sh.
#
# Usage: smoke.sh <off|on> <tag> <worktree> [nsys-graph]
#   off  arm_env defaults with SGLANG_DSV41_ENABLE_LAYER_FUSION=0
#   on   arm_env defaults with SGLANG_DSV41_ENABLE_LAYER_FUSION=1
...
T=/data/models/slang/nvfp4-work/layer-fusion/smoke
OUT=$T/$TAG-$ARM
PORT=30013
...
OVR="{'SGLANG_DSV41_ENABLE_LAYER_FUSION': '$FLAG'}"
...
printf '%s\n' "${ENV[@]}" > $OUT/env.txt
grep -q "^SGLANG_DSV41_ENABLE_LAYER_FUSION=$FLAG$" $OUT/env.txt || { say "flag missing from env; refusing"; exit 1; }
