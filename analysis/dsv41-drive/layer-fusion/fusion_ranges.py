...
Also the kernels per step and per layer (gather to gather). Flag off: A 26, BC 63; flag on: A 1, BC 2.
...
for step in steps:
    ...
    for L, p in enumerate(posts):
        ...
        prev = g
out = {"steps": len(steps), "kernels_per_step_p50": statistics.median(per_step)}
