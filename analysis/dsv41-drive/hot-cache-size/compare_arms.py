"""Compare hot-cache-size smoke arms (copied from ../row-images/compare_arms.py; adds graph_step vram_miss per step).
...
def analyse(arm_dir, include_warmup=False):
    ...
    decode_fwds, steps, graph_ram_rows, graph_vram_rows = set(), 0, 0, 0
    if include_warmup:  # every graph step counts, as wake.py's `forward > 1` filter does
        for i in gblocks:
            for r in blocks[i]["recs"]:
                ...
                graph_vram_rows += r.get("vram_miss", 0)
    per_request = []
    intervals = []
    ...
    return {
        ...
        "graph_vram_miss_per_step": graph_vram_rows / steps if steps else float("nan"),
...
def main():
    ...
    for name, r in res.items():
        ...
        print(f"  demands/step {r['demands_per_step']:.2f}  rows/step {r['rows_per_step']:.2f}  "
              ...
              f"graph_step vram_miss/step {r['graph_vram_miss_per_step']:.2f}")
