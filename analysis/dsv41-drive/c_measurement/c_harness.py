...
class ForeignLoad:
    """Max CPU use of any process that is not ours, over one window, as a percentage of one core."""
    def __init__(self, own_pids):
        self.own = set(own_pids); self.hz = os.sysconf("SC_CLK_TCK")
    def _snap(self):
        out = {}
        for d in os.listdir("/proc"):
            if not d.isdigit() or int(d) in self.own: continue
            try:
                f = Path("/proc/%s/stat" % d).read_text().rsplit(")", 1)[1].split()
                out[int(d)] = (int(f[11]) + int(f[12]), Path("/proc/%s/comm" % d).read_text().strip())
            except (OSError, IndexError): pass
        return out
    def start(self):
        self.a = self._snap(); self.t = time.monotonic()
    def stop(self):
        b = self._snap(); dt = time.monotonic() - self.t; worst = (0.0, "")
        for pid, (j, name) in b.items():
            if pid in self.a:
                pct = 100.0 * (j - self.a[pid][0]) / self.hz / dt
                if pct > worst[0]: worst = (pct, "%s[%d]" % (name, pid))
        return worst
...
def run(args):
    ...
    foreign = _NoForeign() if dry else ForeignLoad([os.getpid()]); smi = None      # a dry run must not depend on the ambient load of the machine it runs on
    if not dry:
    ...
    order_log = []; load_windows = []
    def visit(cell, sink):
        foreign.start(); t0 = time.monotonic()
        T, extra = dev.run_visit(cell, LAUNCHES_PER_CELL // 2, args)
        t1 = time.monotonic(); fp = foreign.stop()
    ...
    for p in range(PASSES):
        ...
        if args.with_nvme:
            ...
            for cell, v in abba(load_cells, rng):
                visit(cell, load_acc[cell]); order_log.append((p, list(cell), v)); load_windows.append((list(cell), load_acc[cell][-1][2], load_acc[cell][-1][3]))
        ...
        for cell, visits in list(acc.items()) + list(load_acc.items()):
            ...
            extra = {"cpu": visits[0][1].get("cpu"), "pass": p, "visits": len(visits)}
