...
Throwaway probe for the doorbell prototype. All copies are host-issued on a side stream; every time is a
CLOCK_MONOTONIC host time at which a CUDA event was first observed complete by a spin loop.

(a) One compute-only graph (no doorbell kernel, no acquire, no poll):
    long_copy_before     copy queued, then the graph replayed (control: overlap expected)
...
(b) K short per-layer graphs replayed back to back:
...
(c) The doorbell between per-layer graphs: graph j = wait(tag j) [j > 0] + compute + post(tag j+1):
    doorbell_layers          replays launched back to back
    doorbell_layers_gated    host waits for the thread to enqueue request j+1 before launching replay j+1

For (a) and (b) the reported hold is how long the copy took beyond an idle copy, and when it completed
relative to the end of the replay that was running when it was queued and to the last replay launched.
"""
...
class Probe:
    ...
    def run_doorbell(self, gated):
        ...
        name = "doorbell_layers_gated" if gated else "doorbell_layers"
        with ExpertDoorbellCopier(segments, rows, max_tags=layers + 1, cpu_core=self.args.spin_core,
            ...
            graphs = [self.capture(lambda index=index: layer(index)) for index in range(layers)]
            ...
            emit({"probe": name, "rows": rows, "layers": layers, "timeouts_per_token": summary(timeouts),
    ...
    def run(self):
        ...
        if not self.args.skip_doorbell:
            self.run_doorbell(gated=False)
            self.run_doorbell(gated=True)
