...
| **T5** | The partial terminal mask names **only** unacknowledged lanes. | GPU. Stage 1 copies and acknowledges; stage 2 fails on an injected read failure. Assert `skipped_mask` has the hit lane's bit **clear** and the miss lane's set, and `kLeaseDoubleSignal == 0`. | Restore the whole-request mask `(1u << named) - 1u` at `:513`. **Resolved 2026-09-22 (T5 written):** the mutant goes red on the mask bit, confirmed. The second detector does **not** fire and the claim that it would was wrong: `retire_leases` early-returns on `lanes_outstanding_ == 0` (`exl3_ram_miss_host.cpp:2212`), and in this minimal request the hit lane is the only outstanding lease, so its own ack drops the count to zero before any later call could observe the wrongly-set bit. Measured `mask = 3`, `lease_double_signal = 0`. The mask assertion alone carries the test; "two independent detectors" is withdrawn as the reason it is non-vacuous. Making the second detector live needs a second, unacknowledged lease to hold `lanes_outstanding_ > 0`; an attempt to do that perturbed the main request (`go_1` 1 -> 0, unexpected `leases_voided`) and is **open**, not dropped. |
...
| **T7** | One request deadline, not one per stage. | Delay both stages to 0.9x the timeout; the request must fail at ~1x, not ~1.8x. | Give each stage its own `start + timeout_ns`. **Resolved 2026-09-22 (T7 written):** killed -- 14.23 s against a 0.56 s bound (`timeout_ms = 400`, bound `1.4x`). The test sets `poll_bound = 100_000_000` deliberately so stage 1 is bounded by the deadline rather than the iteration cap; at a realistic bound stage 1 returns in microseconds and the test would be measuring T10's property instead. **Open:** the magnitude is unexplained. Two independent deadlines predict ~0.8 s, not 14.2 s, so something re-arms or retries beyond a single extra timeout. The qualitative claim is demonstrated; the mechanism behind the 35x is not. One untested hypothesis for whoever picks it up: the test injects `delay_s = 5.0`, so 14.23 s is close to three injected reads (~15 s) and nothing like a multiple of the 400 ms timeout. That would mean the elapsed time is dominated by draining delayed reads, not by deadline arithmetic -- the correct code abandons the request at its one deadline without waiting for the read, while the mutant re-arms and ends up waiting one or more 5 s reads out. If that is right the test still discriminates correctly, but its bound is measuring read drain rather than the second deadline, and the row's "~1x vs ~1.8x" framing does not describe what is actually timed. |
