...
- [x] Record GPU/drive topology, negotiated links, NUMA allocation and first-touch, filesystem alignment, versions, pinned-memory budget, and registration limits. — `50cf23b619`, extended and re-verified against current code in `036900c5cd`: `analysis/dsv41-drive/TOPOLOGY.md`. **Two findings that matter more than the inventory:** the mirrors are not symmetric hardware (`max_sectors_kb` 512 on nvme0 versus 256 on nvme3, which backs /mnt/nvme4; 66 file extents versus 3,112 by `filefrag`), and NVMe completion interrupts for several submitter cores land on cores 64-71 including core 71, so `taskset -c 0-63` does not keep interrupts off production's cores.
  **QUANTIFIED 2026-09-21, because this was being read as more serious than it is.** Measured as a *rate* (per-core `/proc/interrupts` deltas over 10 s under load) rather than from the since-boot totals: **23,669 NVMe interrupts landed on cores 0-63 and 313 on cores 64-71 — 1.3%**, all of them on two queues (IRQs 78 and 80) that are pinned exclusively to the reserved band. The routing is real; at roughly 31 interrupts per second it is not a meaningful load on production's cores, and it does **not** explain those cores running at 91-99% busy (that is the user's own unpinned services). **The gap that does matter is SMT, not interrupts:** cores 28-35 pair with 64-71 and **cpu35's sibling is cpu71 itself**, so occupying cpu35 occupies the physical core the doorbell spins on. A lane obeying `taskset -c 0-63` can therefore still contend directly with production, through execution resources rather than through interrupts. **Method note:** the earlier over-reading came from cumulative counters over a nine-day boot. A total is not a rate -- the same error as reading `ps` `%CPU`, a lifetime average, as current usage.
...
- [x] Allocate two bounded bounce banks and preallocated metadata. Keep SQ/CQ credit handling independent of bank count. Refill submitted I/O before performing bounded packing work; compare a packing worker against the bounded single-owner loop. — Banks and preallocated metadata are in `ddcb0d55ff`; credit independence is pinned by `test_ring_credit_is_independent_of_the_banks`. **The packing-worker comparison has been run** (2026-09-21), under the condition the earlier buffered runs could not cover: O_DIRECT on a real NVMe. `PACK_WORKERS.md`, "The registered O_DIRECT run"; raw data in `analysis/dsv41-drive/pack-workers-odirect/`.

  **Result, against the pre-registered success test (fixed before the run: at c=4, p50 tail at most 0.6x inline's and p90 not above inline's, in two consecutive runs): PASSED in both runs.** Splitting each row across four workers (`4:4`) cuts the exposed tail to 0.8-1.1 ms from inline's 2.7-5.5 ms. It also holds at 1 and 2 rows (0.29x-0.36x), where inline reproduced.

  **Three findings that change what "a packing worker" should mean here.** (1) **The win comes from splitting a row, not from adding workers**: `4:1` (four workers, no split) merely matches inline at 4 rows (0.99x, 0.51x), while `4:4` is 0.41x/0.20x. (2) **A single whole-row worker is worse than inline at 4 rows**, 1.4x-1.8x in both runs, well outside the 10% noise floor; untested why, the likely reason being that one serial worker packs four rows back to back while the owner has stopped packing. (3) **The tail win is bought, not free**: whole-process CPU per request at 4 rows is 34.7/38.8 ms for `4:4` against inline's 22.0/28.5, about 1.4x-1.6x, because the workers spin. On a quiet box that is invisible; under production contention, where the scheduler and tokenizer threads want the same cores, it may not be. Production contention is explicitly not covered.

  **Caveat recorded with the numbers:** the registered precondition (inline's own tail reproduces between the two runs) **failed for the 4-row p50** (2.76 ms vs 5.54 ms) on that first, single-drive run. The 4-row inline tail is bimodal, one row's packing (~2.7 ms) or two rows' (~5.5 ms), and the median straddles the boundary. So **the sign and a floor of at least 2.4x are stable; the exact ratio (0.20x-0.41x) from that run is not.** The verdict did not depend on it: `4:4` cleared the 0.6x bar against 2.758 ms, the lowest inline value seen in any of four runs.

  **Both open items closed 2026-09-21.**

  **(a) Mirror roots on separate drives.** Rerun with mirror 0 on `/mnt/nvme0` and mirror 1 on `/mnt/nvme4` (`nvme3n1`), the drives production actually splits its mirrors across (`bench_pack_workers.py` gained a `--work-dir2` argument for this; omitting it keeps the old one-drive behaviour). Same pre-registered test, unchanged: **PASSED in both runs** (run 1: p50 1.343 <= 0.6 x 5.487 = 3.292, p90 1.685 <= 5.598; run 2: p50 1.182 <= 0.6 x 5.502 = 3.301, p90 1.437 <= 5.549), with more margin than the single-drive run. **The single-drive run's own reproducibility caveat did not reappear**: inline's 4-row p50 reproduced to 0.27% across the two split-drive runs (5.487 vs 5.502 ms), landing consistently in the two-rows-packed regime instead of straddling the boundary. Both drives carried real, roughly equal O_DIRECT traffic (~10.7 GiB read each, matching to within 0.03%), confirming the split mirrors were exercised as production configures them. CPU cost held at the same order of magnitude, 1.65x-1.68x against the single-drive run's 1.4x-1.6x. Full writeup and raw data: `PACK_WORKERS.md` "The split-drive rerun"; `analysis/dsv41-drive/pack-workers-odirect-split/`.

  **(b) The refill-before-packing clause.** `test_refill_submits_a_credit_freed_read_before_the_next_rows_blocking_pack` (`test_exl3_ram_miss_split.py`) pins the claim directly: with credit for two of three single-extent rows and a 150 ms inline pack, a later row's read must submit before an earlier-admitted row's (blocking) pack of a *different* row finishes — not merely before packing starts once. The ordering mutation this guards (moving `refill()` to run after `pack_one()` each turn) was applied on a private worktree, confirmed to make the read fail outright (the loop's own termination check fires before anything is ever submitted), and reverted; the full split/thread/pack_workers suites (539 tests) were re-run clean afterward. Detail: `PACK_WORKERS.md` "Evidence for 'refill submitted I/O before performing bounded packing work'".

  **Landing the measured configuration required no code change.** `RowReader::set_pack` already defaults `pack_split` to `pack_workers` when the caller passes 0, and the only production entry point (`Dsv41Config.ram_miss_pack_workers` -> `Exl3RamMissRowBackend` -> `exl3_ram_miss_open`) passes `pack_workers` alone — there is no separate split knob. So `SGLANG_DSV41_RAM_MISS_PACK_WORKERS=4` already selects exactly the measured winning mode, `4:4`, not the `4:1` mode the registered data found no better than inline. The default stays **0** (off, inline); this is a statement that the wiring already matches the measured result, not a proposal to change what ships. Also stale and now corrected in `PACK_WORKERS.md`: the note gating the flag on the StageRecord carrying its mode is **closed** (schema 5's `pack_workers`/`pack_split` fields already reach the exported trace and `overlap_timeline.py` already refuses worker-mode overlap metrics), so enabling the flag in a traced run is no longer a silent-misread hazard.

  **Still not covered by any of this:** production contention (the packing workers ran on otherwise-idle cores in both O_DIRECT runs; the scheduler and tokenizer threads were not competing for them), and anything past the CPU/host-copy boundary — no GPU, no H2D transfer.
...
> **State as of 2026-09-21: the device half of the lease protocol now exists, and none of the boxes below moves.**
> Until today the host side was complete and polling for acknowledgements no device could ever write: `kLeaseLaneAck`
> and `kLeaseTerminal` were read by `retire_leases()` and written nowhere, and `go_count`/`lane_ctx`/`kEpoch` existed
> nowhere in the tree. `70847da92f` lands `LEASE_PROTOCOL.md` 7.3 (the wait kernel: validates each lane's `RowResult`,
> commits `go_count`, publishes `Terminal` on refusal) and 7.4 (the acknowledgement kernel, a separate kernel after the
> copy per 6.4), plus the minimum post-kernel support they need (`kEpoch`/`kPendingEpoch`, the `LaneRequest` write, and
> arming every `count > 0` record in lease mode, which is section 15's rule and not a choice).
>
> **Verified** (ledger and limits in `LEASE_PROTOCOL.md` 20.2l, reproduced independently on a fresh worktree at the
> branch tip): 31/31 lease-kernel GPU tests on the 5090; **694 passed, 0 failed** across the full `exl3` CPU suite, so
> the lease-off path is behaviourally unchanged; 26 layout-agreement tests; and **16 of 16 mutants killed**, including
> the minimal single-spurious-acknowledgement case that backs 7.4's "a skipped copy emits no acknowledgement by
> construction".
>
> **Why no box is ticked.** Every box below names a *service-side* behaviour -- the shutdown drain, the terminal
> cancellation handshake, admission pressure under a delayed GPU consumer, Option F's ordering. The two kernels make
...
- [x] Initially exercise the acknowledgements after the existing whole-request copy, without changing its scheduling. Include RAM hits as well as newly read rows in source ownership.
  - **Artefacts:** the wait and acknowledgement kernels (`70847da92f`, LEASE_PROTOCOL 7.3/7.4); the backend and service wiring behind `SGLANG_DSV41_ENABLE_RAM_MISS_LEASES` (`49784e1105`, off by default); the multi-layer graph tests (`40c1189cc5`).
  - **Scheduling is unchanged:** `Exl3RamMissRowBackend.post` with a lease block runs translate, the copy with `go_count` in place of `plan.count`, then `ack(keep)` in that same stream. The ack is a separate kernel launched after the copy, never fused, so every source load has retired before the ack store (§6.4). Without a lease block it is the inherited `post` verbatim.
  - **RAM hits are in source ownership, and this is measured, not argued:** `open11_serving_path.py` reports `rows_read_during_timing: 0` in all four views -- every timed request is all-hit -- while the same run grants and acknowledges **36,264 leases, 0 voided, 0 double-signalled**. Leases are therefore taken on rows already resident, not only on newly read ones. The arming rule that produces this is §15's, `need_count > 0 || advise != 0 || (lease != nullptr && planned_count > 0)`.
  - **Independently reproduced** (2026-09-21, not by the author, from a pulled `wt-dsv41` under `gpu-run.sh`): `test_exl3_lease_kernels_cuda.py` 32/32; `test_exl3_ram_miss_graph_gpu.py` 11/11, including six multi-layer tests on their first execution; byte-identical output leases-off vs leases-on across 4 and 20 layers, 20 wrapping the 16-deep `kDemandRecords`/`kLeaseRing` four times within one replay.
  - **Seen to fail:** three backend mutants were each killed -- no ack; the copy using `plan.count` instead of `go_count`; host lease mode never enabled. The `plan.count` mutant **survived the first assertion set** and was killed only after a scratch-rows-untouched check was added, so the kill is a real discrimination rather than a restatement.
  - **The multi-layer tests were then mutated, and they discriminate.** Two mutants of the ack kernel's generation tag (`exl3_ram_miss.cuh:549`), each applied on `wt-dsv41`, run, and reverted without being committed:
