...
- [ ] Allocate two bounded bounce banks and preallocated metadata. Keep SQ/CQ credit handling independent of bank count. Refill submitted I/O before performing bounded packing work; compare a packing worker against the bounded single-owner loop. — **partial.** Banks and preallocated metadata are in `ddcb0d55ff`; credit independence is pinned by `test_ring_credit_is_independent_of_the_banks`. **The packing-worker comparison has now been run** (2026-09-21), under the condition the earlier buffered runs could not cover: O_DIRECT on a real NVMe. `PACK_WORKERS.md`, "The registered O_DIRECT run"; raw data in `analysis/dsv41-drive/pack-workers-odirect/`.
  ...
  **Caveat recorded with the numbers:** the registered precondition (inline's own tail reproduces between the two runs) **failed for the 4-row p50** (2.76 ms vs 5.54 ms). The 4-row inline tail is bimodal, one row's packing (~2.7 ms) or two rows' (~5.5 ms), and the median straddles the boundary. So **the sign and a floor of at least 2.4x are stable; the exact ratio (0.20x-0.41x) is not.** The verdict does not depend on it: `4:4` clears the 0.6x bar against 2.758 ms, the lowest inline value seen in any of four runs.

  **Still open on this box, which is why the item is not ticked:** both mirror roots sat on one drive (production splits them), so read-completion timing is less realistic than production's, though the cold-versus-warm DRAM mechanism under test does not depend on it; and the "refill submitted I/O before performing bounded packing work" clause of this item has no cited evidence of its own.
...
- [x] Initially exercise the acknowledgements after the existing whole-request copy, without changing its scheduling. Include RAM hits as well as newly read rows in source ownership.
  ...
  - **Limitation, recorded rather than left implicit:** the six multi-layer tests have **not** been mutated. They pass, and their anti-vacuity guard is two-sided (each layer must hold a route expert that is neither VRAM-hot nor tier-resident before the replay, and the rows the service actually reads must equal that count exactly), but no mutant is yet known that ring-wrap catches and the single-layer tests miss. Until one exists, "20 layers pass" means the path runs correctly, not that the test would notice if it stopped.
