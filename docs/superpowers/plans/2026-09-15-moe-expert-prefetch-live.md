...
0. **Bus-bound vs latency-bound is unresolved (crypto-c9, measured facts, 2026-09-15).** Prefetch hides latency; it never reduces bytes. If serving is bus-bound, every saving in Task 3 shrinks.
   ...
   - **The saving is bounded by idle bus time per token.** Prefetch redistributes a fixed byte budget, so it can hide at most what the link is not already carrying: `saving <= step_time - bytes_per_token / 12.08 GB/s`. At ~3.15 misses/layer (~417 MB/token) a ~50 ms step is ~69% busy, leaving room for Task 3's 5-7 ms/token. At the 64% warming miss rate the bus is saturated and the saving goes to ~0. **Phase B must compute this duty cycle from the measured steady-state numbers before it commits a budget.**
