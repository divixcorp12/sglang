...
def test_breakable_decode_turns_the_alt_stream_overlap_off(model_dir, multi_stream_unset):
    # Captured in a breakable decode graph, DSV4's alt-stream overlap yields wrong
    # attention output (graph_parity on the truncated model: the tokens diverge at
    # the first decode step); with it off, graph and eager agree bit for bit.
