...
def _stage_record(**over):
    record = dict(
        ...
        retried_bytes=0, cancelled_bytes=0, rows_untraced=0, extents_untraced=0, dropped_before=0,
        row_pack=[
...
def test_ram_miss_requests_are_traced_and_skipped_by_tier_sim(tmp_path):
    ...
    assert first["request"] == {"seq": 7, "type": "demand", "ok": 1, "rows": 2, "batches": 1, "backlog": 0}
